...
class GraphToTensor:
    ...
    FASTA_PATH = "../dbs/generated/sequences.fasta"
    
    def __init__(self, negative_interaction_method='most_distant'):
        self.df = self.load_data()
        self.protein_sequences = self.read_sequences_from_fasta(self.FASTA_PATH)
    ...
    def load_data(self):
        ...
        df = df[['Interactor 1 RefSeq id', 'Interactor 2 RefSeq id']]
    ...
    def extract_node_features(self, G):
        features_list = ['basic_protein_properties', 'secondary_structure_content', 'other_properties', 'amino_acid_composition']
    ...
    def split_graph(self, train_size=0.8):
        train_graph, test_graph = self.split_graph_into_train_and_test(train_size)
        return self.graph_to_graph_tensor(train_graph), self.graph_to_graph_tensor(test_graph)

    def split_graph_into_train_and_test(self, train_size):
